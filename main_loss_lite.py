...
from model import loss_stacked
...
tf.flags.DEFINE_string("network_name", "loss_stacked", "the name og this network")
...
def main(args=None):
    ...
    with tf.Graph().as_default():
        ...
        with tf.name_scope(name='Network'):
            ...
            prediction = tf.argmax(input=logits_up, asix=3)
        ...
        with tf.name_scope(name='Others'):
            # Accuracy
            valid_accuracy = tf.metrics.accuracy(labels=next_y, predictions=prediction)
            # Graph Logs
