...
from model import dense_dilated
...
tf.flags.DEFINE_string("logs_dir", "./logs_dense_dilated", "path to logs directory")
...
def main(args=None):
    ...
    with tf.Graph().as_default():
        ...
        with tf.name_scope(name='Network'):
            network_name = 'dense_dilated'
            # Inference
            with tf.variable_scope(network_name, reuse=False):
                logits, logits_up = dense_dilated(
            ...
            with tf.variable_scope(network_name, reuse=True):
                logits_infer, logits_infer_up = dense_dilated(
            ...
            loss = tf.reduce_mean((tf.nn.sparse_softmax_cross_entropy_with_logits(
                logits=logits, labels=tf.squeeze(next_y_scale_down, axis=3), name="loss")))
            # Optimizer
