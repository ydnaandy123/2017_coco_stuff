...
from model import ae_lite_dilated
...
def main(args=None):
    ...
    predict_downscale, bottle_downscale = 2, 8
    """
    Read coco parser     
    """
    ...
    with tf.Graph().as_default():
        ...
        with tf.name_scope(name='Network'):
            ...
            with tf.variable_scope(FLAGS.network_name, reuse=False):
                logits_stacks, logits_up = ae_lite_dilated(
            ...
            with tf.variable_scope(FLAGS.network_name, reuse=True):
                logits_stacks_test, logits_up_test = ae_lite_dilated(
        ...
        with tf.name_scope(name='Optimize'):
            ...
            loss_stack_class1 = tf.nn.softmax_cross_entropy_with_logits(
