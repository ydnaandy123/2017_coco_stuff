...
from model import ae_lite_se_final, get_shape
...
tf.flags.DEFINE_float("epsilon", "1e-08", "epsilon")
    ...
    with tf.Graph().as_default():
        ...
        with tf.name_scope(name='Network'):
            ...
            with tf.variable_scope(FLAGS.network_name, reuse=False):
                logits_stacks, logits_up = ae_lite_se_final(
            ...
            with tf.variable_scope(FLAGS.network_name, reuse=True):
                logits_stacks_test, logits_up_test = ae_lite_se_final(
        ...
        with tf.name_scope(name='Optimize'):
            ...
            with tf.control_dependencies(update_ops):
                train_op = tf.train.AdamOptimizer(learning_rate=learning_rate, beta1=beta1, epsilon=epsilon).minimize(
        ...
        with tf.Session() as sess:
            ...
            """
            if FLAGS.mode == 'train':
                print('Training mode! Batch size:{:d}, Learning rate:{:f}, Beta1:{:f}, Epsilon:{:f}'.format(
                    FLAGS.batch_size, learning_rate, beta1, epsilon))
