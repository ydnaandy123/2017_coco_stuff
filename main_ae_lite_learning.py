...
from model import ae_lite_full
...
tf.flags.DEFINE_float("beta1", "0.5", "beta1")
...
def main(args=None):
    ...
    predict_downscale, bottle_downscale = 1, 64
    """
    Read coco parser     
    """
    ...
    with tf.Graph().as_default():
        ...
        with tf.name_scope(name='Network'):
            ...
            with tf.variable_scope(FLAGS.network_name, reuse=False):
                logits_stacks, logits_up = ae_lite_full(
            ...
            with tf.variable_scope(FLAGS.network_name, reuse=True):
                logits_stacks_test, logits_up_test = ae_lite_full(
        ...
        with tf.name_scope(name='Optimize'):
            ...
            loss = tf.nn.softmax_cross_entropy_with_logits(
                labels=next_y_one_hot_class[:, :, :, 92:],  logits=logits_stacks[0][:, :, :, :],
                name='loss_stack_class1')
            # loss_stack_class2 = tf.nn.softmax_cross_entropy_with_logits(
            #     labels=next_y_one_hot_class[:, :, :, 92:],  logits=logits_stacks[1][:, :, :, :],
            #     name='loss_stack_class2')

            # loss = loss_stack_class1
            ...
            with tf.control_dependencies(update_ops):
                train_op = tf.train.AdamOptimizer(learning_rate=learning_rate, beta1=beta1).minimize(
        ...
        with tf.Session() as sess:
            ...
            """
            if FLAGS.mode == 'train':
                print('Training mode! Batch size:{:d}, Learning rate:{:f}, Beta1:{:f}'.format(
                    FLAGS.batch_size, learning_rate, beta1))
