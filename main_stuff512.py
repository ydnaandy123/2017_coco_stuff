...
from model import full_512

FLAGS = tf.flags.FLAGS
tf.flags.DEFINE_integer("image_height", "512", "image target height")
tf.flags.DEFINE_integer("image_width", "512", "image target width")
tf.flags.DEFINE_integer("num_of_feature", "3", "number of feature")
tf.flags.DEFINE_integer("num_of_class", "92", "number of class")

tf.flags.DEFINE_string("network_name", "full_512", "the name og this network")
    ...
    with tf.Graph().as_default():
        ...
        with tf.name_scope(name='Network'):
            ...
            with tf.variable_scope(FLAGS.network_name, reuse=False):
                logits_stacks, logits_up = loss_stacked(
            ...
            with tf.variable_scope(FLAGS.network_name, reuse=True):
                logits_stacks_test, logits_up_test = loss_stacked(
        ...
        with tf.name_scope(name='Optimize'):
            ...
            next_y_scale_down = tf.image.resize_nearest_neighbor(
                ...
                [FLAGS.image_height // fraction_num, FLAGS.image_width // fraction_num])
            next_y_scale_down = tf.squeeze(next_y_scale_down, axis=3)
            # Make label one-hot
            next_y_one_hot = tf.one_hot(next_y_scale_down, FLAGS.num_of_class, axis=-1)
            '''
            loss_sum = 0
            ...
            loss_stack1 = tf.nn.softmax_cross_entropy_with_logits(
                labels=next_y_one_hot[:, :, :, 1:], logits=logits_stacks[0][:, :, :, 1:], name='loss_stack1')
            loss_stack2 = tf.nn.softmax_cross_entropy_with_logits(
                labels=next_y_one_hot[:, :, :, 1:], logits=logits_stacks[1][:, :, :, 1:], name='loss_stack2')
            loss = 0.4 * loss_stack1 + 0.6 * loss_stack2
        ...
        with tf.name_scope(name='Others'):
            ...
            update_op, valid_accuracy = tf.metrics.accuracy(
                labels=next_y, predictions=prediction)
            # Graph Logs
        ...
        with tf.Session() as sess:
            ...
            if FLAGS.mode == 'train':
                ...
                coco_parser.inference_with_tf(sess=sess, logits_up_test=logits_up_test, test_x=test_x,
