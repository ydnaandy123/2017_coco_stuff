...
from model import class_super, get_shape
...
tf.flags.DEFINE_string("network_name", "class_super", "the name og this network")
...
def main(args=None):
    ...
    predict_downscale, bottle_downscale = 4, 64
    """
    Read coco parser     
    """
    ...
    with tf.Graph().as_default():
        ...
        with tf.name_scope(name='Network'):
            ...
            with tf.variable_scope(FLAGS.network_name, reuse=False):
                logits_stacks, logits_up = class_super(
            ...
            with tf.variable_scope(FLAGS.network_name, reuse=True):
                logits_stacks_test, logits_up_test = class_super(
        ...
        with tf.name_scope(name='Optimize'):
            ...
            loss = 0.6 * loss_stack_class_sup_sup + 0.5 * loss_stack_class_sup + 0.4 * loss_stack_class
