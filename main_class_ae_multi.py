...
from model import ae_multi, get_shape
...
tf.flags.DEFINE_string("network_name", "class_ae_multi", "the name og this network")
    ...
    with tf.Graph().as_default():
        ...
        with tf.name_scope(name='Network'):
            ...
            with tf.variable_scope(FLAGS.network_name, reuse=False):
                logits_stacks, logits_up = ae_multi(
            ...
            prediction_sup_sup = tf.argmax(tf.image.resize_bilinear(logits_stacks[3], get_shape(next_x)[1:3]), axis=3)
            prediction_sup = tf.argmax(tf.image.resize_bilinear(logits_stacks[4], get_shape(next_x)[1:3]), axis=3)
            # Testing with arbitrary shape (used for testing) (use placeholder)
            with tf.variable_scope(FLAGS.network_name, reuse=True):
                logits_stacks_test, logits_up_test = ae_multi(
        ...
        with tf.name_scope(name='Optimize'):
            ...
            loss_stack_class_sup_sup_2 = tf.nn.softmax_cross_entropy_with_logits(
                labels=next_y_one_hot_class_sup_sup[:, :, :, 1:],  logits=logits_stacks[3][:, :, :, :],
                name='loss_stack_class_sup_sup')
            loss_stack_class_sup_2 = tf.nn.softmax_cross_entropy_with_logits(
                labels=next_y_one_hot_class_sup[:, :, :, 1:],  logits=logits_stacks[4][:, :, :, :],
                name='loss_stack_class_sup')
            loss_stack_class_2 = tf.nn.softmax_cross_entropy_with_logits(
                labels=next_y_one_hot_class[:, :, :, 92:],  logits=logits_stacks[5][:, :, :, :],
                name='loss_stack_class')

            loss = loss_stack_class_sup_sup + loss_stack_class_sup + loss_stack_class + \
                   loss_stack_class_sup_sup_2 + loss_stack_class_sup_2 + loss_stack_class_2
