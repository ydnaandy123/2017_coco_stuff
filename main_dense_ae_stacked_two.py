...
from model import dense_ae_stacked_wo
...
tf.flags.DEFINE_string("network_name", "dense_ae_stacked_wo", "the name og this network")
    ...
    with tf.Graph().as_default():
        ...
        with tf.name_scope(name='Network'):
            ...
            with tf.variable_scope(FLAGS.network_name, reuse=False):
                logits_stacks, logits_up = dense_ae_stacked_wo(
            ...
            with tf.variable_scope(FLAGS.network_name, reuse=True):
                logits_stacks_test, logits_up_test = dense_ae_stacked_wo(
        ...
        with tf.name_scope(name='Optimize'):
            ...
            loss = 0.3 * loss_stack1 + 0.3 * loss_stack2 + 0.4 * loss_stack3
