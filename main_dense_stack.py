...
from model import stuff512_test
...
tf.flags.DEFINE_string("network_name", "UUU", "the name og this network")
tf.flags.DEFINE_integer("num_epochs", "50", "number of epochs for training")
tf.flags.DEFINE_integer("batch_size", "12", "batch size for training")
...
def main(args=None):
    ...
    import numpy as np
    x = np.array(Image.open('249_1_0_rgb.png').resize((224, 224), resample=Image.BILINEAR))
    ...
    with tf.Graph().as_default():
        ...
        with tf.name_scope(name='Network'):
            ...
            with tf.variable_scope(FLAGS.network_name, reuse=True):
                logits_stacks_test, logits_up_test = fcn(
        ...
        with tf.Session() as sess:
            ...
            if FLAGS.mode == 'train':
                ...
                observe_freq, val_freq, val_max_num = 150, 400, 40
                """
                Iterators
                """
