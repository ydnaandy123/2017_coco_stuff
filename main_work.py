    ...
    with tf.Graph().as_default():
        ...
        with tf.name_scope(name='Input'):
            ...
            training_dataset = coco_parser.tfrecord_get_dataset(
                ...
                super_class=False, shuffle_size=None)
        ...
        with tf.name_scope(name='Network'):
            ...
            # prediction_sup_sup = tf.argmax(tf.image.resize_bilinear(logits_stacks[1], get_shape(next_x)[1:3]), axis=3)
            # prediction_sup = tf.argmax(tf.image.resize_bilinear(logits_stacks[2], get_shape(next_x)[1:3]), axis=3)
            # Testing with arbitrary shape (used for testing) (use placeholder)
            with tf.variable_scope(FLAGS.network_name, reuse=True):
                logits_stacks_test, logits_up_test = work(
        ...
        with tf.name_scope(name='Optimize'):
            # Label scale down
            d_sh, d_sw = FLAGS.image_height // predict_downscale, FLAGS.image_width // predict_downscale
            d_sh2, d_sw2 = d_sh // 2, d_sw // 2
            d_sh4, d_sw4 = d_sh // 4, d_sw // 4
            d_sh8, d_sw8 = d_sh // 8, d_sw // 8

            # next_y_scale_down = tf.image.resize_nearest_neighbor(tf.expand_dims(next_y, axis=3), [d_sh, d_sw])
            ...
            next_y_one_hot_class = tf.one_hot(next_y, 184, axis=-1)
            next_y_scale_down = tf.image.resize_nearest_neighbor(next_y_one_hot_class, [d_sh, d_sw])
            # next_y_scale_down2 = tf.image.resize_nearest_neighbor(next_y_scale_down, [d_sh2, d_sw2])
            # next_y_scale_down4 = tf.image.resize_nearest_neighbor(next_y_scale_down2, [d_sh4, d_sw4])
            # next_y_scale_down8 = tf.image.resize_nearest_neighbor(next_y_scale_down4, [d_sh8, d_sw8])

            # Tensor cross_entropy
            '''
            loss_stack_class1 = tf.reduce_mean(tf.nn.softmax_cross_entropy_with_logits(
                labels=next_y_scale_down8[:, :, :, 92:],  logits=logits_stacks[0][:, :, :, :],
                name='loss_stack_class1'))
            loss_stack_class2 = tf.reduce_mean(tf.nn.softmax_cross_entropy_with_logits(
                labels=next_y_scale_down4[:, :, :, 92:],  logits=logits_stacks[1][:, :, :, :],
                name='loss_stack_class2'))
            loss_stack_class3 = tf.reduce_mean(tf.nn.softmax_cross_entropy_with_logits(
                labels=next_y_scale_down2[:, :, :, 92:],  logits=logits_stacks[2][:, :, :, :],
                name='loss_stack_class3'))
            '''
            loss_stack_class4 = tf.nn.softmax_cross_entropy_with_logits(
                labels=next_y_scale_down[:, :, :, 92:],  logits=logits_stacks[0][:, :, :, :],
                name='loss_stack_class4')

            loss = loss_stack_class4

            # Weighted loss (Not calculate unassigned label '0')
            ignored_pixels = next_y_scale_down[:, :, :, 0]
