        ...
        if FLAGS.mode == 'train':
            ...
            for epoch in range(0, epochs):
                ...
                for batch in range(0, batches):
                    x_batch, y_batch = coco_parser.load_train_datum_batch(batch*batch_size, (batch+1)*batch_size)
                    x_batch = np.array(x_batch, dtype=np.float32)
                    y_batch_ori = np.array(y_batch, dtype=np.int32)
                    y_batch = y_batch_ori
                    y_batch[np.nonzero(y_batch < 92)] = 91
                    y_batch -= 91
                    ...
                    if global_step_sess % 1000 == 1:
                        ...
                        for batch_idx, train_path in \
                            ...
                            scipy.misc.imsave('{}/images/{:d}_{}_0_rgb.png'.format(
                                FLAGS.logs_dir, global_step_sess, name), x_batch[batch_idx])
                            ...
                            scipy.misc.imsave('{}/images/{:d}_{}_3_gt.png'.format(
                                FLAGS.logs_dir, global_step_sess, name), y_batch_ori[batch_idx])
                            pred_reverse = np.argmax(logits_sess[batch_idx], axis=2) + 91
                            pred_reverse[np.nonzero(pred_reverse <= 91)] = 0
                            scipy.misc.imsave('{}/images/{:d}_{}_4_pred.png'.format(
                                FLAGS.logs_dir, global_step_sess, name), pred_reverse)
