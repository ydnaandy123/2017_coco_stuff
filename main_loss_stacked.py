...
from model import dense_stacked
...
tf.flags.DEFINE_string("logs_dir", "./logs_dense_stacked", "path to logs directory")
...
def main(args=None):
    ...
    with tf.Graph().as_default():
        ...
        with tf.name_scope(name='Network'):
            network_name = 'dense_stacked'
            # Inference
            with tf.variable_scope(network_name, reuse=False):
                logits_semi, logits, logits_up = dense_stacked(
            ...
            with tf.variable_scope(network_name, reuse=True):
                logits_infer_semi, logits_infer, logits_infer_up = dense_stacked(
                    x=data_x, flags=FLAGS, is_training=False, drop_probability=0.0)
            # Loss
            next_y_scale_down = tf.image.resize_nearest_neighbor(
                tf.expand_dims(next_y, axis=3), [FLAGS.image_height // 4, FLAGS.image_width // 4])
            next_y_scale_down = tf.squeeze(next_y_scale_down, axis=3)
            loss_semi = tf.reduce_mean((tf.nn.sparse_softmax_cross_entropy_with_logits(
                logits=logits_semi, labels=next_y_scale_down, name="loss_semi")))
            loss_final = tf.reduce_mean((tf.nn.sparse_softmax_cross_entropy_with_logits(
                logits=logits, labels=next_y_scale_down, name="loss_final")))
            loss = loss_semi + loss_final
            # Optimizer
        ...
        with tf.Session() as sess:
            ...
            if FLAGS.mode == 'train':
                ...
                events_freq, save_freq = 20, 1000
                observe_freq, val_freq, val_max_num = 1500, 2500, 250
                """
                Iterators
                """
                    ...
                    while True:
                        try:
                            ...
                            _, loss_sess, global_step_sess, summary_loss_sess, \
                                next_x_sess, next_y_sess, logits_up_sess \
                                = sess.run([train_op, loss, global_step, summary_loss,
                                            next_x, next_y, logits_up], feed_dict=feed_dict_train)
                            ...
                            if True and global_step_sess % observe_freq == 1:
                                ...
                                coco_parser.visualize_data(
                                    ...
                                    global_step=global_step_sess, pred_batches=logits_up_sess,
                            ...
                            if True and global_step_sess % val_freq == (val_freq-1):
                                ...
                                while True:
                                    try:
                                        # Logging the events
                                        loss_sess, next_x_sess, next_y_sess, logits_up_sess = \
                                            sess.run([loss, next_x, next_y, logits_up], feed_dict=feed_dict_infer)
                                        print(
                ...
                coco_parser.inference_with_tf(sess=sess, logits_infer=logits_infer_up, data_x=data_x,
                                              drop_probability=drop_probability, is_training=is_training,
                                              is_dev=True, pool_times=5)

            elif FLAGS.mode == 'test':
                coco_parser.inference_with_tf(sess=sess, logits_infer=logits_infer_up, data_x=data_x,
                                              drop_probability=drop_probability, is_training=is_training,
                                              is_dev=False, pool_times=5)
